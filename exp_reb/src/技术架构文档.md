...
class Node:
    ...
    def deploy(self, service_id: str, instances: int, cpu_per_instance: float):
...
class DeploymentPlan:
    ...
    def validate(self, topology: 'Topology', services: Dict[str, MicroService]) -> bool:
        ...
        for node_id, node in topology.nodes.items():
            used = self.get_node_cpu_usage(node_id, services)
...
class FirstFitDecreasing(DeploymentAlgorithm):
    ...
    def deploy(self, topology: Topology,
        ...
        for service_id, demand in sorted_services:
            ...
            for node_id, node in topology.nodes.items():
                if node.can_deploy(demand["cpu"]):
                    plan.add(service_id, node_id, demand["instances"])
                    # 更新节点状态
                    node.deploy(service_id, demand["instances"], cfg.cpu_per_instance)
                    break
...
DEFAULT_CONFIG = {
    ...
    # 服务
    "num_services": 10,
    "cpu_per_instance_range": [1, 4],  # 核心数
