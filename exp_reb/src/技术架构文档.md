...
class OurAlgorithm(DeploymentAlgorithm):
    ...
    核心思想:
    - 根据当前负载和约束，动态选择合适的模型版本
    - 高负载 → 选择 Model-L (高μ, 低精度, 快速处理)
    - 低负载 → 选择 Model-H (低μ, 高精度, 资源充裕)
    - 中等负载 → 选择 Model-M (平衡)
    """
    ...
    def __init__(self, config: Optional[Dict] = None):
        ...
        self.load_threshold_high = config.get("load_threshold_high", 0.7)
        self.load_threshold_low = config.get("load_threshold_low", 0.3)
    ...
    def deploy(self, topology: Topology,
        ...
        # Step 1: 分析每条链的负载
        chain_loads = self._analyze_chain_loads(chains, services)

        # Step 2: 为每个服务选择模型版本
        service_versions = {}
        for chain in chains:
            for service_id in chain.services:
                load = chain_loads.get(service_id, 0.5)
                version = self._select_version_based_on_load(
                    service_id, load, services, chain
                )
                service_versions[(service_id, chain.chain_id)] = version

        # Step 3: 综合所有链的需求，生成部署方案
        # (简化：使用贪心策略选择节点)
        deployment_decisions = self._generate_deployment(
            service_versions, services, topology, chains
        )

        for (service_id, node_id, version, count) in deployment_decisions:
            plan.add(service_id, node_id, version, count)

        return plan

    def _select_version_based_on_load(self, service_id: str, load: float,
                                     services: Dict[str, Service],
                                     chain: ServiceChain) -> str:
        """
        基于负载选择模型版本

        负载高 → Model-L (快), 负载低 → Model-H (准)
        """
        # 获取服务配置
    ...
    def _analyze_chain_loads(self, chains, services) -> Dict[str, float]:
        """分析每个服务的负载 (简化: 使用到达率/μ的比例)"""
        loads = {}
        for chain in chains:
            for service_id in chain.services:
                # 简化: 负载 = 到达率 / 默认μ
                mu_default = services[service_id].get_version("Model-M").mu
                loads[service_id] = chain.arrival_rate / mu_default
        return loads
```
