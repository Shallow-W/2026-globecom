...
def _make_cfg(n_slots: int = 50) -> Config:
    cfg = Config()
    cfg.n_slots = n_slots
    cfg.slot_duration_s = 0.01  # 0.01 最快
...
def run_perturb(perturb_type: str = "ar", cfg: Config = None):
    ...
    perturb_type:
        'ar'   - 到达率扰动 (arrival_rate): [1, 2, 3, 4, 5, 6, 7]
        'len'  - 请求长度扰动 (request_length): [4,5,6,7,8,9]
        'task' - 请求类型数扰动 (n_task_types): [1, 2, 3, 4,5,6]
    """
    if cfg is None:
        cfg = _make_cfg(30)

    perturb_map = {
        "ar": ("arrival_rate", [1, 2, 3, 4, 5, 6, 7], "到达率扰动 (arrival_rate)"),
        "len": ("request_length", [4, 5, 6, 7, 8, 9], "请求长度扰动 (request_length)"),
        "task": ("n_task_types", [1, 2, 3, 4, 5, 6], "请求类型数扰动 (n_task_types)"),
    }
...
def run_equal_latency(target_latency: float = 50.0, cfg: Config = None):
    ...
    run_equal_latency_experiment(
        cfg, target_latency=target_latency, topology_scale="small"
    )
...
def run_all(cfg: Config = None):
    ...
    print("\n--- 请求长度扰动 ---")
    run_perturbation_experiment(
        cfg, perturb="request_length", values=[2, 5, 10, 20, 50]
    )
    print("\n--- 请求类型数扰动 ---")
